...
<div class="appcontainer">
  ...
  <div class="flow-edit">
    <div class="flow-edit-content">
      <edit-flow ref="flowEdit" v-show="false"></edit-flow>
  ...
  methods: {
    getData() {
      this.axios.get('/api/users/getList').then(res => {
        if (res.data.data.ok) {
          let dataNode = res.data.data.nodeList;
          let dataLine = res.data.data.lineList;
          let dataNode1 = [];
          let dataLine1 = [];
          ...
          this.data.nodeList = dataNode1;
          this.data.lineList = dataLine;
          this.jsPlumb = jsPlumb.getInstance();
          this.$nextTick(() => {
            this.init();
          });
    ...
    loadEasyFlow() {
      ...
      console.log('aaa',this.data.lineList);
      // 初始化连线
    ...
    editLine(conn) {
      ...
      _this.data.lineList.forEach(function (item, index) {
        if (item.from == conn.sourceId && item.to == conn.targetId) {
          ...
          _this.editType = 'line';
          _this.currentLine.isFlag = !isFlag;
