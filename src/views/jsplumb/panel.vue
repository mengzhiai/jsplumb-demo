      ...
      <el-row>
        ...
        <el-col :span="24">
          <div style="margin-bottom: 5px; margin-left: 10px">
            ...
            <el-button type="info" icon="el-icon-document" size="small" @click="dataInfo">流程数据</el-button>
            <!-- <el-button type="primary" @click="dataReloadA" icon="el-icon-refresh">流程A</el-button> -->
            ...
            <el-button type="warning" @click="changeLabel" size="small" icon="el-icon-edit-outline">设置线</el-button>
          </div>
        </el-col>
      </el-row>
      <el-row>
        ...
        <el-col :span="24">
          <div id="flowContainer" class="container">
            <template v-for="node in data.nodeList">
              <flow-node v-show="node.show" :id="node.id" :node="node" @deleteNode="deleteNode" @changeNodeSite="changeNodeSite" @nodeRightMenu="nodeRightMenu" @editNode="editNode">
              </flow-node>
            </template>
          </div>
        </el-col>
...
export default {
  ...
  methods: {
    getList() {
      this.axios.get('/api/users/getList').then(res => {
        console.log(res.data.data.nodeList);
        let obj = {
          nodeList: res.data.data.nodeList,
          lineList: res.data.data.lineList
        }
        this.jsPlumb = jsPlumb.getInstance()
        this.$nextTick(() => {
          // 默认加载流程A的数据、在这里可以根据具体的业务返回符合流程数据格式的数据即可
